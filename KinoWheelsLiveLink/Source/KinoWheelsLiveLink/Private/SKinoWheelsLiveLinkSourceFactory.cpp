...
#define LOCTEXT_NAMESPACE "KinoWheelsLiveLinkSourceEditor"
		...
		[
			...
			[
				...
				[
					...
					.OnTextCommitted(this, &SKinoWheelsLiveLinkSourceFactory::OnEndpointChanged)
				]
			]
			+ SVerticalBox::Slot()
			.HAlign(HAlign_Right)
			...
			[
				SNew(SButton)
				.OnClicked(this, &SKinoWheelsLiveLinkSourceFactory::OnOkClicked)
				[
					SNew(STextBlock)
					.Text(LOCTEXT("Ok", "Ok"))
				]
...
void SKinoWheelsLiveLinkSourceFactory::OnEndpointChanged(const FText& NewValue, ETextCommit::Type)
{
...
FReply SKinoWheelsLiveLinkSourceFactory::OnOkClicked()
{
	TSharedPtr<SEditableTextBox> EditabledTextPin = EditabledText.Pin();
	if (EditabledTextPin.IsValid())
	{
